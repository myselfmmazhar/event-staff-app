...
/**
 * Scheduled/actual shift instants are stored as wall-clock-in-UTC. Read/write
 * `<input type="datetime-local">` via UTC components so the wall-clock the
 * admin typed round-trips unchanged regardless of viewer timezone.
 */
...
export function InvoiceForm({ invoice }: InvoiceFormProps) {
    ...
    const submitActionRef = useRef<"save" | "print" | "download">("save");
    ...
    const createMutation = trpc.invoices.create.useMutation({
        onSuccess: (data) => {
            ...
            if (action === "print") {
                router.push(`/invoices/${data.id}?action=print`);
            } else if (action === "download") {
                router.push(`/invoices/${data.id}?action=download`);
            } else {
    ...
    const updateMutation = trpc.invoices.update.useMutation({
        onSuccess: () => {
            ...
            const action = submitActionRef.current;
            if (action === "print") {
                router.push(`/invoices/${invoice.id}?action=print`);
            } else if (action === "download") {
                router.push(`/invoices/${invoice.id}?action=download`);
            } else {
                router.push("/invoices");
            }
        },
    ...
    const actionButtons = (
        <div className="flex flex-wrap gap-3">
            ...
            <Button
                ...
                variant="outline"
                disabled={createMutation.isPending || updateMutation.isPending}
                onClick={() => { submitActionRef.current = "print"; }}
                className="gap-2"
            >
            ...
            <Button
                type="submit"
                disabled={createMutation.isPending || updateMutation.isPending}
                onClick={() => { submitActionRef.current = "download"; }}
                className="gap-2"
            >
